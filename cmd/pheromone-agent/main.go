// pheromone-agent is the Pheromone agent process.
//
// Usage:
//
//	pheromone-agent [--config-path <dir>]
//	pheromone-agent config validate [--config-path <dir>]
//	pheromone-agent config generate [--config-path <dir>] [--format json|yaml] [--component agent|skill|all]
package main
...
const usageText = `pheromone-agent — Pheromone agent process

Usage:
  pheromone-agent [global flags] <command> [command flags]
...
Global Flags:
  --config-path <dir>   Directory containing agent configuration files
                        (default: /etc/pheromone/agent, env: PHEROMONE_AGENT_CONFIG_PATH)

Config Generate Flags:
  --format    <fmt>     Output format: json or yaml (default: yaml)
  --component <name>    Component to generate config for:
                        agent, skill, all (default: all)
  --output    <path>    Output file path (default: <config-path>/<component>.<format>)

Examples:
  # Validate the current agent configuration
  pheromone-agent config validate

  # Generate a default agent config in YAML
  pheromone-agent config generate --format yaml --component agent

  # Generate all component configs in JSON to a custom path
  pheromone-agent config generate --format json --component all --config-path /opt/pheromone/agent
`
...
func run(args []string) int {
	...
	// Global flags
	globalFlags := flag.NewFlagSet("pheromone-agent", flag.ContinueOnError)
...
func runConfigValidate(args []string, globalConfigPath string) int {
	...
	log.Info("validating agent configuration", "path", *configPath)
	...
	log.Info("configuration is valid",
		...
		"server_addr", cfg.Agent.ServerAddr,
...
func runConfigGenerate(args []string, globalConfigPath string) int {
	...
	component := fs.String("component", "all", "component to generate: agent, skill, all")
	...
	ops := map[string]generateOp{
		"agent": {"agent", config.GenerateAgentConfig},
		"skill": {"skill", config.GenerateSkillConfig},
	}
	...
	op, ok := ops[*component]
	if !ok {
		log.Error("unknown component", "component", *component, "valid", "agent, skill, all")
		return 1
	}
...
func configPathFromEnv() string {
	if v := os.Getenv("PHEROMONE_AGENT_CONFIG_PATH"); v != "" {
		return v
	}
